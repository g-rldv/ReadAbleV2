...
// Soft pastels, responsive layout, global accessibility settings
// ============================================================
...
import {
  ...
  Mail, Lock, User, ArrowLeft, ShieldCheck,
  Users, Sparkles, GraduationCap, Home, Baby,
...
function AuthInput({ label, type = 'text', value, onChange, placeholder, name, icon: Icon, error }) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: 'block', fontSize: 12, fontWeight: 700, marginBottom: 5, color: C.textSecondary }}>
        {label}
      </label>
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{
            ...
            borderRadius: 12,
            border: `1.5px solid ${error ? '#F09090' : C.border}`,
            background: '#FAFAFE',
            color: C.textPrimary,
            fontSize: 14,
...
function SoftButton({ children, onClick, type = 'button', disabled, color, outline, style: extraStyle = {}, small, className = '' }) {
  ...
  const filled = { ...base, background: hov && !disabled ? `${color}DD` : color, color: '#FFFFFF' };
  const outlineStyle = { ...base, background: hov && !disabled ? `${color}10` : 'transparent', color: color || C.primary };
...
function TextLink({ onClick, children }) {
  return (
    <button onClick={onClick} style={{ fontSize: 12, fontWeight: 800, color: C.primary, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
      {children}
    </button>
  );
}
  ...
  if (step === 'email') return (
    <div>
      ...
      <form onSubmit={sendOTP}>
        ...
        <SoftButton type="submit" disabled={loading || !email.trim()} color={C.primary} style={{ width: '100%', marginTop: 4 }}>
    ...
    <div>
      ...
      <div style={{ textAlign: 'center', marginTop: 10 }}>
        {resendCd > 0 ? <span style={{ fontSize: 12, color: C.textMuted }}>Resend in {resendCd}s</span> : (
          <button onClick={sendOTP} style={{ fontSize: 12, fontWeight: 700, color: C.primary, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
            Resend code
          </button>
        )}
      </div>
          ...
          <>
            ...
            <form onSubmit={submit}>
              ...
              <div style={{ textAlign: 'right', marginTop: -8, marginBottom: 14 }}>
                <button type="button" onClick={() => setShowForgot(true)} style={{ fontSize: 12, fontWeight: 700, color: C.primary, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                  Forgot password?
                </button>
              </div>
            ...
            <ModalFooter>
              No account? <TextLink onClick={onSwitchToRegister}>Create one free</TextLink>
            </ModalFooter>
          </>
        )}
      </ModalCard>
        ...
        {loading ? (
          ...
          <>
            ...
            <form onSubmit={submitForm}>
              ...
              <SoftButton type="submit" color={roleColor} style={{ width: '100%' }}>
          ...
          <>
            <button onClick={() => { setStep('form'); setOtp(''); setOtpErr(''); }} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: C.textMuted, background: 'none', border: 'none', cursor: 'pointer', padding: '0 0 14px', fontFamily: 'inherit' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: 12 }}>
              {resendCd > 0 ? <span style={{ fontSize: 12, color: C.textMuted }}>Resend in {resendCd}s</span> : (
                <button onClick={resendOTP} style={{ fontSize: 12, fontWeight: 700, color: C.primary, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                  Resend code
                </button>
              )}
            </div>
...
function SettingsModal({ onClose }) {
  const { settings, setTheme, setTextSize, setBgMusicEnabled, setTtsEnabled } = useSettings();
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={{ width: '100%', maxWidth: 448, borderRadius: 18, background: '#181334', border: '1px solid #4B3E72', boxShadow: '0 18px 60px rgba(0,0,0,0.35)', overflow: 'hidden', color: '#EDE8FF', animation: 'modalPop 0.22s ease-out' }}>
        <div style={{ height: 62, padding: '0 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #3D3260' }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#A9A0C8', cursor: 'pointer', padding: 4 }}>
            <X size={20} />
          </button>
        </div>
          ...
          {tabs.map(({ key, label, Icon }) => {
            ...
            return (
              <button
                key={key}
                onClick={() => setTab(key)}
                style={{ height: 56, border: 'none', background: 'transparent', color: active ? '#65C7FF' : '#A9A0C8', borderBottom: active ? '2px solid #65C7FF' : '2px solid transparent', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11, fontWeight: 800, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4 }}
              >
                <Icon size={16} />
                {label}
              </button>
                ...
                {THEME_OPTIONS.map(({ key, label, Icon }) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ minHeight: 70, borderRadius: 14, border: `2px solid ${active ? '#58B9FF' : '#55476E'}`, background: active ? '#1F2D5B' : '#0E0B21', color: active ? '#7FD2FF' : '#C7B8F5', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 800, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}
                    >
                      <Icon size={18} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {TEXT_OPTIONS.map(size => (
                  <button
                    ...
                    style={{ padding: '15px 12px', borderRadius: 14, border: `2px solid ${settings.text_size === size.key ? '#58B9FF' : '#55476E'}`, background: settings.text_size === size.key ? '#1F2D5B' : '#0E0B21', color: '#C7B8F5', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 900 }}
                  >
                    {size.label}
                  </button>
                ))}
...
function SettingToggle({ title, subtitle, enabled, onClick, Icon }) {
  return (
    <button
      onClick={onClick}
      style={{ width: '100%', minHeight: 92, borderRadius: 16, border: `2px solid ${enabled ? '#58B9FF' : '#55476E'}`, background: enabled ? '#1F2D5B' : '#0E0B21', color: '#EDE8FF', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: 18 }}
    >
...
function Nav({ onSignIn, onOpenSettings }) {
  return (
    <nav className="landing-nav" style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--bg-card, #FFFFFF)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border-color, #DDD8F2)', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
        <div style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--accent, #5A50A0)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <BookOpen size={18} color="#FFFFFF" />
        </div>
        <span className="landing-logo-text" style={{ fontFamily: '"Fredoka One", cursive', fontSize: 22, color: 'var(--text-primary, #28264A)', whiteSpace: 'nowrap' }}>ReadAble</span>
      </div>
      ...
      <div className="landing-nav-actions" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <button
          ...
          style={{ width: 38, height: 38, borderRadius: 12, border: '1px solid var(--border-color, #DDD8F2)', background: 'var(--bg-primary, #F2F0FA)', color: 'var(--text-muted, #6A6898)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
        >
          <SettingsIcon size={18} />
        </button>
        <SoftButton className="landing-nav-signin" onClick={onSignIn} outline color={C.primary} small>Sign In</SoftButton>
      </div>
    </nav>
  );
}
...
function Hero({ onTeacher, onParent, onSignIn }) {
  return (
    <section className="landing-hero" style={{ padding: '56px 24px 0', maxWidth: 680, margin: '0 auto', textAlign: 'center', position: 'relative' }}>
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 20, background: '#EDE8FF', border: '1px solid #C8C0F0', marginBottom: 18 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 800, color: '#6050B0' }}>Designed for learners with ASD</span>
      </div>

      <h1 className="landing-hero-title" style={{ fontFamily: '"Fredoka One", cursive', fontSize: 'clamp(34px, 6vw, 52px)', lineHeight: 1.1, color: C.textPrimary, margin: '0 0 14px' }}>
        Reading made gentle.
        <br />
        <span style={{ color: '#6050A8' }}>Progress made visible.</span>
      </h1>

      <p className="landing-hero-copy" style={{ fontSize: 16, color: C.textSecondary, lineHeight: 1.6, maxWidth: 480, margin: '0 auto 32px', fontWeight: 500 }}>
      ...
      <div className="landing-hero-actions" style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center', marginBottom: 16 }}>
        <SoftButton className="landing-action-btn" onClick={onTeacher} color={C.teacher.accent}>
          <GraduationCap size={16} /> I'm a Teacher
        </SoftButton>
        <SoftButton className="landing-action-btn" onClick={onParent} color={C.parent.accent}>
          <Home size={16} /> I'm a Parent
        </SoftButton>
        <SoftButton className="landing-action-btn" onClick={onSignIn} outline color={C.primary}>
          Sign In
        </SoftButton>
      </div>
...
function AudienceCard({ Icon, title, subtitle, description, points, cta, scheme, onCta, isStudent }) {
  ...
  return (
    <div
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{ background: hov ? scheme.cardBg : C.white, border: `1.5px solid ${hov ? scheme.border : C.border}`, borderRadius: 20, overflow: 'hidden', boxShadow: hov ? C.shadowMd : C.shadowSm, transition: 'all 0.2s', display: 'flex', flexDirection: 'column', transform: hov ? 'translateY(-2px)' : 'none' }}
    >
      <div style={{ background: scheme.pageBg, borderBottom: `1px solid ${scheme.border}`, padding: '20px 22px 16px' }}>
        ...
        <p style={{ fontSize: 12, fontWeight: 700, color: scheme.accent, margin: 0, textTransform: 'uppercase' }}>{subtitle}</p>
      </div>
  ...
  const roles = [
    {
      ...
      points: ['Build custom story assessments at 4 difficulty levels', 'Track literal, inference, vocabulary and emotion skills', 'Generate and send progress reports to parents', 'Manage classrooms with simple 6-character join codes'],
...
function FeaturesSection() {
  ...
  return (
    <section className="landing-section" style={{ padding: '0 24px 64px', maxWidth: 1100, margin: '0 auto' }}>
      <div className="features-panel" style={{ borderRadius: 24, background: C.white, border: `1px solid ${C.border}`, padding: '40px 32px', boxShadow: C.shadowSm }}>
        ...
        {steps.map(s => (
          <div key={s.num} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: '18px 20px', boxShadow: C.shadowSm }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <div style={{ width: 30, height: 30, borderRadius: 10, background: s.scheme.pageBg, border: `1px solid ${s.scheme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <p style={{ fontSize: 11, fontWeight: 800, color: s.scheme.accent, textTransform: 'uppercase', margin: '0 0 3px' }}>{s.who}</p>
...
function StudentPreviewStrip() {
  return (
    <section className="landing-section" style={{ padding: '0 24px 64px', maxWidth: 1100, margin: '0 auto' }}>
      <div className="student-preview-panel" style={{ borderRadius: 24, background: C.student.pageBg, border: `1.5px solid ${C.student.border}`, padding: '36px 36px 32px', display: 'flex', flexWrap: 'wrap', gap: 28, alignItems: 'center' }}>
        <div style={{ flex: '1 1 280px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 20, background: C.student.accentLight, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 800, color: C.student.textDark }}>STUDENT MODE</span>
          </div>
        ...
        <div className="student-preview-card" style={{ flex: '1 1 260px', background: C.white, borderRadius: 18, border: `1.5px solid ${C.student.border}`, padding: 20, boxShadow: C.shadowMd }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              <div key={label} title={label} style={{ flex: 1, padding: '10px 0', borderRadius: 12, background: C.student.pageBg, border: `1.5px solid ${C.student.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function Footer({ onTeacher, onParent, onSignIn }) {
  return (
    <footer className="landing-footer" style={{ background: '#2A2848', padding: '56px 24px', textAlign: 'center' }}>
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: '"Fredoka One", cursive', fontSize: 28, color: '#FFFFFF', margin: '0 0 10px', lineHeight: 1.2 }}>Ready to begin?</h2>
        ...
        <div className="landing-footer-actions" style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
          <SoftButton className="landing-action-btn" onClick={onTeacher} color={C.teacher.accent}>
          ...
          <SoftButton className="landing-action-btn" onClick={onSignIn} outline color="#8080C0">
            Sign In
          </SoftButton>
        </div>
        <p style={{ fontSize: 11, color: '#8080C0', marginTop: 28 }}>
...
function SectionLabel({ icon, text }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 20, background: '#EDE8FF', border: '1px solid #C8C0F0', marginBottom: 10 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 800, color: '#6050B0', textTransform: 'uppercase' }}>{text}</span>
    </div>
  );
}
    ...
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      @media (max-width: 520px) {
        ...
        .features-panel > div:last-child > div {
...
export default function LandingPage() {
  ...
  return (
    <>
      <GlobalStyles />
      <div style={{ fontFamily: '"Nunito", sans-serif', background: 'var(--bg-primary, #F2F0FA)', minHeight: '100vh', color: 'var(--text-primary, #28264A)' }}>
        <Nav
